...
export function getLineItemPrice<T extends LineItemWithProduct>(
  ...
  let productUnitPrice = getUnitPriceWithFields(
    lineItem.expand!.product,
    lineItem.fields,
    withDiscount,
  )

  return lineItem.quantity * productUnitPrice
}

export const validDiscount = writable(false)
export function getUnitPriceWithFields<TProduct extends ProductResponse>(
...
export function getProductFieldPriceIncrease<T extends ProductFieldTyped>(
  productField?: T,
  option?: string,
) {
...
export function getCartWeight<T extends LineItemWithProduct>(cart: T[]) {
  return _.sumBy(
    cart,
    (item) => (item.expand!.product.weightInOz ?? 0) * item.quantity,
  )
}
